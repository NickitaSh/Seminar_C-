...
//Метод исключения из массива строки и столбца по координатам минимального элемента
...
int[,] arr2D = Fill2DArray(row, column, 0, 99); //Заполнение массива 
Print2DArrayColor(arr2D); //Печать двумерного цветного массива 
Console.WriteLine(); //Перенос на новую строку
(int x, int y) minElem = SearchMinElementIn2DArray(arr2D); //Поиск минимального элемента
int[,] out2DArr = Converted2DArray(arr2D, minElem.x, minElem.y); //Исключение из массива строки и столбца по координатам минимального элемента
Print2DArrayColor(out2DArr); //Печать двумерного цветного измененного массива